# -*- coding: utf-8 -*-
"""
CVD Machine Learning 9.0
This is the exact same as 8, I want to verify that our random seed produces the same results.
...
callbacks = [
    keras.callbacks.ModelCheckpoint("SavedModelOutputs/SavesModel9/save_at_{epoch}.h5"),
...
plot_metrics(history, "SavedModelOutputs/SavesModel9/history")


#Write out the saved metrics to a new file.
with open("SavedModelOutputs/SavesModel9/Evaluated", "w") as file:
...
model.save("SavedModelOutputs/SavesModel9/first_save.h5")


#Plot a confusion matrix of the current model.
predictions = model.predict(test_ds)
plot_cm(test_labels, predictions, p=0.5, savePath="SavedModelOutputs/SavesModel9/ConfusionMatrix1")
...
model_freezed.save('SavedModelOutputs/SavesModel9/last_save.h5')

#Calls function in functions.py. Save a plot of precision, recall, PRC and loss through the epochs
plot_metrics(history1, "SavedModelOutputs/SavesModel9/history1")

with open("SavedModelOutputs/SavesModel9/EvaluatedFineTuning", "w") as file:
    results = model.evaluate(test_ds)
    for name, value in zip(model.metrics_names, results):
        file.write(str(name)+': '+str(value)+"\n")

newPredict = model.predict(test_ds)
plot_cm(test_labels, newPredict, p=0.5, savePath="SavedModelOutputs/SavesModel9/ConfusionMatrix2")