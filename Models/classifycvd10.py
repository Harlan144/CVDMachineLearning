# -*- coding: utf-8 -*-
"""
CVD Machine Learning 10.0
Model modifies 9.0 to allow for a larger image size (224,224) vs (512,512)
Also, increased epoch count to 50 from 30.
...
image_size = (512, 512) #Changed from (224,224)
batch_size = 32
...
callbacks = [
    keras.callbacks.ModelCheckpoint("SavedModelOutputs/SavesModel10/save_at_{epoch}.h5"),
    keras.callbacks.EarlyStopping(
    monitor='val_auc', #Stops when AUROC is the highest in the validation set.
    verbose=1,
    patience=10,
    mode='max',
...
plot_metrics(history, "SavedModelOutputs/SavesModel10/history")


#Write out the saved metrics to a new file.
with open("SavedModelOutputs/SavesModel10/Evaluated", "w") as file:
...
model.save("SavedModelOutputs/SavesModel10/first_save.h5")


#Plot a confusion matrix of the current model.
predictions = model.predict(test_ds)
plot_cm(test_labels, predictions, p=0.5, savePath="SavedModelOutputs/SavesModel10/ConfusionMatrix1")
...
model_freezed.save('SavedModelOutputs/SavesModel10/last_save.h5')

#Calls function in functions.py. Save a plot of precision, recall, PRC and loss through the epochs
plot_metrics(history1, "SavedModelOutputs/SavesModel10/history1")

with open("SavedModelOutputs/SavesModel10/EvaluatedFineTuning", "w") as file:
    results = model.evaluate(test_ds)
    for name, value in zip(model.metrics_names, results):
        file.write(str(name)+': '+str(value)+"\n")

newPredict = model.predict(test_ds)
plot_cm(test_labels, newPredict, p=0.5, savePath="SavedModelOutputs/SavesModel10/ConfusionMatrix2")